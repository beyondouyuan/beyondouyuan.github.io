...
>
>
grails 也是一个很不错的mvc框架，医用grails可以让你在不到一分钟的时间内，仅用一两条命令即可搭建起一个包含各种依赖目录的工程文件，可惜的是，这个框架实在是太小众了。
>
...
启动后在浏览器上访问[localhost:3000](localhost:3000)页面，起所显示的内容其实就是在router/index.js上的内容渲染到views后所得到的。
